...
Ext.calendar.DoctorScheduler = Ext.extend(Ext.Panel, {
    initComponent: function(){
        this.startHour = WebApp.settings.startHour; //эти настройки отвечают за количество часов, отображаемых
        this.endHour = WebApp.settings.endHour;//в дне или неделе. Обрабатывается в файле DayBodyTemplate и DayBodyView
        ...
        this.staffStore = new Ext.data.JsonStore({
            ...
            //data: calendarList, // defined in calendar-list.js
            proxy: new Ext.data.HttpProxy({
                url: App.utils.getApiUrl('staff','staff'),
                method:'GET'
            }),
            autoLoad: true,
            fields: [
        ...
        this.eventStore = new Ext.data.Store({
            ...
            proxy: new Ext.data.HttpProxy({
                url: App.utils.getApiUrl('scheduler','event'),
                method:'GET'
            }),
...
Ext.reg('scheduler',Ext.calendar.DoctorScheduler);


App.webapp.actions.add('doctorscheduler', new Ext.Action({
    text: 'Календарь',
    scale: 'medium',
    handler: function(){
        WebApp.fireEvent('launchapp','scheduler',{
            closable:true,
            doctorMode:true
        });
    }
...
App.webapp.actions.add('scheduler', new Ext.Action({
    text: 'Календарь',
    scale: 'medium',
    handler: function(){
        WebApp.fireEvent('launchapp','scheduler');
