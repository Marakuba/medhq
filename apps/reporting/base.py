...
class Report():
    ...
    def make(self):
        ...
        list_results = map(lambda x: dict(map(lambda y: (y[0], self.dfields[y[0]]['renderer'](y[1], x) if y[0] in self.dfields and 'renderer' in self.dfields[y[0]] else y[1]), x.items())), dict_result)
...
#        self.results = map(lambda x:map(lambda y:self.fields[y]['renderer'](x[y],x) if isinstance(self.fields[y],dict) else x[y],x),dict_result)
    ...
    def as_list(self, node=None, header=True):
        ...
        general_list = []
        #Заголовки таблицы
        headers_data = dict(map(lambda x: (x['name'], x['verbose']) if isinstance(x, dict) and 'verbose' in x else (x['name'], x['name']) if isinstance(x, dict) else (x, x), self.fields))
        ...
        if isinstance(node, RootNode) and 'aggr' in self.totals and node.aggr_val.items():
...
#            pdb.set_trace()
            ...
            if 'position' not in self.totals or self.totals['position'] in ('top', 'both'):
                general_list.append(item_totals)
        #Данные
        general_list += self.get_group_data(node)

        if isinstance(node, RootNode) and 'position' in self.totals and self.totals['position'] in ('bottom', 'both'):
            general_list.append(item_totals)
