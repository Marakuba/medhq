...
//          split:true,
    ...
    editCard: function(record){
        if (!record.data.executed) {
            var orderId = App.utils.uriToId(record.data.ordered_service);
            ...
            this.orderStore.load({callback: function(records){
                if (!records[0]){
                    ...
                    config = {
                        ...
                        baseServiceId:App.utils.uriToId(orderRecord.data.service),
                        print_name:record.data.service_name,
                        record:record
                    };

                    WebApp.fireEvent('launchapp', 'cardapp',config);
                }
            },scope:this});
...
Ext.reg('conclusionapp', App.examination.ConclApp);


App.webapp.actions.add('conclusionapp', new Ext.Action({
    text: 'Архив заключений',
    scale: 'medium',
    handler: function(){
        WebApp.fireEvent('launchapp','conclusionapp');
    }
}));