...
App.dict.XGlossaryTree = Ext.extend(Ext.ux.tree.RemoteTreePanel, {
	...
	initComponent: function(){
        ...
        this.subNodeBtn = new Ext.Button({
...
//                contextMenu:this.contextMenu,
				...
				tbar:[this.insertBtn, this.subNodeBtn, '-',this.editBtn],
				tools:[{
					 id:'refresh',
					handler:function() {
...
//		console.log(this.actionNode);
		...
		jsonData['resource_uri'] = App.App.getApiUrl('examination','glossary',node.attributes.id);
		jsonData['section'] = this.section;
		jsonData['staff'] = this.staff;
		jsonData['base_service'] = this.base_service;
		...
		var o = Ext.apply(this.getOptions(), {
			...
			method:'PUT',
			jsonData:data,
			url:App.App.getApiUrl('examination','glossary', node.attributes.id),
			headers:{
				'Content-Type':'application/json'
			},
	...
	moveNode:function(e,movedNode) {
		...
		jsonData['resource_uri'] = App.App.getApiUrl('examination','glossary', movedNode.attributes.id);
		jsonData['section'] = this.section;
		jsonData['staff'] = this.staff;
		jsonData['base_service'] = this.base_service;
		...
		var o = Ext.apply(this.getOptions(), {
			...
			node:e.dropNode,
			params:params,
			url:App.App.getApiUrl('examination','glossary',movedNode.attributes.id),
			method:'PUT',
			jsonData:data,
			headers:{
				'Content-Type':'application/json'
	...
	actionCallback:function(options, success, response) {
		...
		switch(options.action) {
			...
			case 'insertChild':
				...
				options.node.attributes['resource_uri'] = App.getApiUrl('examination','glossary')+'/'+o.objects.id;
				options.node.select();
			break;
		}
		//}}}
				...
				Ext.each(s,function(v){
					if (v){
						...
						node.expand(false, false, function(n) {
							if(true === insert) {
								child = n.insertBefore(this.loader.createNode({text:v, loaded:true}), n.firstChild);
							}
							else {
								child = n.parentNode.appendChild(this.loader.createNode({text:v, loaded:true}));
							}
						}.createDelegate(this));
