...
App.laborder.LabTestGrid = Ext.extend(Ext.grid.GridPanel, {

	initComponent : function() {

		this.backend = App.getBackend('labtest');
...
//		    	hidden:true,
		...
		var config = {
			id:'lab-test-grid',
			title:'Журнал заказов по услугам',
			loadMask : {
...
//				" <span style='padding-left:10px; color:gray;font-variant:italic'>{[values.rs[0].data['laboratory']]}</span>"
		...
		App.laborder.LabTestGrid.superclass.initComponent.apply(this, arguments);
		App.eventManager.on('globalsearch', this.onGlobalSearch, this);
		this.initToolbar();
		//this.on('rowcontextmenu', this.onContextMenu, this);
		this.on('afterrender', function() {
			var date = new Date();
			this.storeFilter('created__gte', date.format('Y-m-d 00:00'), false);
...
Ext.reg('labtestgrid',App.laborder.LabTestGrid);
