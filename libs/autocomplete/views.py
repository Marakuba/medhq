...
class AutoComplete(object):
    ...
    def __call__(self, request, ac_name, query_param='q'):
        ...
        return HttpResponse("%s" % simplejson.dumps(result),
...
class AutoCompleteExt(object):
    ...
    def __call__(self, request, ac_name, query_param='q'):
        ...
        if isinstance(label, basestring):
            ...
            result = []
            for obj in qs:
                result.append({'id':getattr(obj, key), 'value':label(obj)})
        ext_dict = {'success': True, 'data': result, 'message': 'Something good happened on the server!'}
        return HttpResponse("(%s)" % simplejson.dumps(ext_dict),
                mimetype='application/json')
...
autocompleteExtJS = AutoCompleteExt()
